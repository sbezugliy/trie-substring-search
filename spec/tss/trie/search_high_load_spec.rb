...
#     let!(:dictionary) { File.open(File.join(fixt_path, 'dictionary.dat')).read.split(' ') }
#     let!(:health_data) { File.open(File.join(fixt_path, 'health_data.dat')).read.split(' ').map(&:to_i) }
#     let!(:sentences) { File.open(File.join(fixt_path, 'sentences.dat')).read.split("\n").map { |s| s.split(' ') } }
...
#         healthes << act.parse(sentence[2]).map { |w| w[:indexes] }.flatten.reduce(0) do |acc, n|
